...
OBS_STEER_LIMIT   = 0.25   # 장애물 추종 조향 제한. BLOCKED_TURN 제거 후 약간 상향
...
SEARCH_ARC_STEER    = 0.30 # 양쪽 측면 기준 장애물이 없을 때 항상 우회전 원호 탐색
SEARCH_ARC_SPEED    = 0.35 # 원호 탐색 속도
ARC_VFH_TRIGGER     = 500.0 # 원호 탐색 중 이 거리 안에 장애물이 있으면 일반 VFH 안전회피
...
def _search_without_wall_drive(ser, ls=None):
    """wall-follow를 쓰지 않는 상황의 선택기.

    - 양쪽 모두 장애물: 통로 상황 → 일반 VFH
    - 양쪽 모두 없음: 기준 벽 없음 → 우회전 원호 탐색 + VFH 안전회피
    """
    ls = _lidar_read() if ls is None else ls
    if not ls['has_data']:
        return _arc_search_drive(ser)
    ...
    # 이 함수가 한쪽만 장애물인 상황에서 호출되면, 상위 상태가 곧 wall-follow로 재선택하게 한다.
    return _arc_search_drive(ser)
...
def _obstacle_wall_follow_search(ser, follow_ctrl: dict):
    """
    ...
    if mode == 'INIT':
        ...
        if side is None:
            return _search_without_wall_drive(ser, ls)
    ...
    if dist >= OBS_FIND_DIST:
        ...
        follow_ctrl['lost_start'] = None
        follow_ctrl['last_steer'] = 0.0
        return _search_without_wall_drive(ser, _lidar_read())
...
def main():
    ...
    print("=" * 60)
    print("  Camera_v11 | one-side wall-follow + right arc search + VFH")
        ...
        # ③ 미탐지 → VFH 탐색 ────────────────────────────────────────────
            ...
            if w is not None:
                ...
                if elapsed < COLOR_MEMORY_TIME:
                    ...
                    mem_invis_count = 0
                    log = _obstacle_wall_follow_search(ser, follow_ctrl)
